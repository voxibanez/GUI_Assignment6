...
function createTable(startPrice, endPrice, priceStep, startMPG, endMPG, MPGStep) {
  ...
  var websiteframe = tblBody2.innerHTML;
  var tabs = $("#tabs").tabs();
  var tabCounter = parseFloat(document.getElementById("count").innerHTML);
  var id = "tabs" + tabCounter
  //tabTemplate = "<li><a href='#{href}'>#{label}</a> <span class='ui-icon ui-icon-close' onclick= \"function rem(){$(#{id}).remove();};rem();return false;\" role = 'presentation' > Remove Tab < /span></li > ";
  tabTemplate = "<li><a href='#{href}'>#{label}</a>";

  var label = "Grid" + tabCounter,
  ...
  tabs.tabs("refresh");
  document.getElementById("count").innerHTML = tabCounter + 1;
}
